...
| 🍞 Toast feedback on save/update/delete (UX-001) | `<ToastProvider>` mounted in App.jsx + project-create / project-edit success toasts + error toast on validation/SSRF failure | 🟨 (UI: `toast-feedback-ui.spec.mjs` covers `NewProject.jsx` create + edit success toasts via `role="status"` + error path via `role="alert"`. Settings-section toasts — Agent Roles, AI Providers, Integrations, Members, Account — remain 🟥 pending the Tier-1 backfill PR.) |
