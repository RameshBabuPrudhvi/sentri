...
| 🍞 Toast feedback on save/update/delete (UX-001) | `<ToastProvider>` mounted in App.jsx + project-create / project-edit success toasts + error toast on validation/SSRF failure + Settings invite + account export | 🟨 (UI: `toast-feedback-ui.spec.mjs` covers `NewProject.jsx` create + edit success toasts via `role="status"`, SSRF error path via `role="alert"`, `MembersSection` invite via `role="status"`, and `AccountSection` export via `page.waitForEvent('download')` + `role="status"`. Remaining Settings-section toasts — Agent Roles, AI Providers, Integrations (GitHub install ID), `/projects/:id/settings/*` panels — remain 🟥 pending the Tier-1 backfill PR, which needs provider-key / role-config / GitHub-app fixtures first.) |
