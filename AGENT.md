...
| `src/queryClient.js` | Shared TanStack Query `queryClient`, `*QueryKeys` constants, `invalidateProjectDataCache()` / `invalidateDashboardCache()` / `invalidateRunCache(runId)` / `invalidateSettingsCache()` helpers | Bust cached queries after mutations; import the matching `*QueryKeys` when defining a new cached query |
...
- **Per-user audit trail** ✅: Every activity log entry records `userId` and `userName` (migration 001 — columns included in initial schema). The `actor(req)` utility in `utils/actor.js` extracts identity from `req.authUser` (JWT payload includes `name`). Bulk approve/reject/restore actions log per-test entries with the acting user's identity.
