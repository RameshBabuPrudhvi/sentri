...
- **Refactor** — Test Lab page (`frontend/src/pages/TestLab.jsx`) trimmed by ~450 lines via extraction of presentational sub-components and pure helpers (audit §3.1). `ProjIcon`, `PipelinePanel` + `PIPELINE_STAGES`, `LiveLog`, and `QueueRow` now live under `frontend/src/components/test-lab/`; sessionStorage persistence helpers (`loadPersistedRun` / `persistRun` / `clearPersistedRun` / `STORAGE_KEY` / `LOG_CAP`) move to `frontend/src/utils/testLabPersistence.js`; attachment MIME guard + size caps (`isTextMime` / `ACCEPTED_EXTENSIONS` / `MAX_ATTACHMENT_SIZE` / `MAX_TOTAL_ATTACHMENT`) move to `frontend/src/utils/testLabAttachments.js`. `QueueTab` now imports `QueueRow`, `EmptyState`, and the `Clock` icon directly instead of receiving them via DI props. No behaviour change — pure decomposition.
