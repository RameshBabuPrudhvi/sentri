...
### Added
- **Runs**: Embedded Playwright trace viewer (DIF-005) — backend now serves a self-hosted `/trace-viewer/` bundle copied during `npm install`, and Run Detail includes a new **🔍 Open Trace** action that opens the viewer preloaded with the run's signed trace URL. The existing ZIP download remains available as fallback.
