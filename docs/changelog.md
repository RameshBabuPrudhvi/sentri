...
- **UX-001** — Restore success/failure toast feedback across every user-initiated mutation in the frontend. Previously, panel toast callbacks on `/automation`, `/projects/:id/settings/*`, and decomposed `frontend/src/features/settings/sections/*` were wired to `addNotification()` (the notification bell), so saving Auto-Approval threshold, Quality Gates, Web Vitals, Coverage, Agent Roles, AI Providers, Integrations, Members, or Account settings gave no visible confirmation. Many user-initiated mutations on `/projects/:id`, `/projects/new`, `/tests`, `/tests/:id`, `/review-queue`, `/runs/:id`, `/test-lab`, `/approvals`, and `/audit-log` were also silent (no success feedback; errors only surfaced via `console.error`, `alert()`, or local banners). Introduced a global `ToastContext` (`frontend/src/context/ToastContext.jsx`) mounted in `App.jsx` above `<Routes>` and migrated every `api.update*` / `api.create*` / `api.delete*` / `api.run*` / `api.abort*` / `api.approve*` / `api.reject*` / `api.restore*` / `api.revoke*` / `api.rotate*` / `api.invite*` / `api.set*` callsite to emit a visible toast on both success and error. The notification bell is retained for durable async events (SSE `run-complete`, scheduled triggers). Toast component has `role="status"` / `role="alert"` + `aria-live` for screen-reader announcements and a user-dismissable × button (WCAG 2.2 SC 2.2.1).
...
- **Refactor** — Test Lab page (`frontend/src/pages/TestLab.jsx`) trimmed from 1,907 → ~995 lines (~48% reduction, audit §3.1). Pass 1 extracted presentational helpers — `ProjIcon`, `PipelinePanel` + `PIPELINE_STAGES`, `LiveLog`, `QueueRow` (under `frontend/src/components/test-lab/`); sessionStorage helpers (`loadPersistedRun` / `persistRun` / `clearPersistedRun` / `STORAGE_KEY` / `LOG_CAP`) into `frontend/src/utils/testLabPersistence.js`; attachment MIME guard + size caps into `frontend/src/utils/testLabAttachments.js`. Pass 2 split the three biggest remaining render blocks: `<TestLabTopBar>`, `<TestLabProjectSidebar>`, and `<TestLabRunCenter>` (the ~215-line IIFE for the attached-run middle column). Pass 3 extracted three logic seams into their own modules: `useTestLabAttachments` (file upload + Jira import state machine + binary-detection / MIME guard), `useProjectEnvironments` (DIF-012 per-project env list fetch with viewer-403 swallow + project-switch reset), and `splitGeneratedOutcome` (the drafts-vs-auto-approved derivation that was a 30-line `useMemo` body). `ProjIcon` avatar palette now driven by a `--avatar-h` CSS custom property — no raw HSL strings inline (STANDARDS.md:383). `QueueTab` imports `QueueRow`, `EmptyState`, and `Clock` directly instead of receiving them via DI props. Page render now reads as a state machine — `activeRun ? <RunCenter /> : <ConfigPanel />`. The G9 parallel-runs SSE handler + state Maps remain in-file pending that migration's completion. No behaviour change — pure decomposition.
