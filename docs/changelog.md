...
### Added
- **MNT-001** — Vision-based locator healing. Adds a two-tier host-side fallback (stage 7 pixelmatch, stage 8 LLM vision) that fires only after every DOM strategy in the runtime waterfall has failed. New `tryVisionHeal()` in `selfHealing.js` orchestrates both stages with dependency-injected pixelmatch + LLM-vision deps so unit tests stay deterministic (no real CV, no real network in CI). New `callVisionModel()` in `aiProvider.js` covers the Anthropic / OpenAI / Google / OpenRouter / OpenAI-compat multimodal message-shape differences behind a single provider-agnostic call and ties usage into INF-007's token telemetry. `STRATEGY_VERSION` bumped 3 → 4 (existing healing hints are invalidated automatically — the next run retraverses the waterfall to relearn). New `Vision Healing` tab in the Quality card (per-project mode picker with `off` / `pixelmatch_only` / `pixelmatch_and_llm`, daily call cap, monthly cost cap; `pixelmatch_and_llm` server-gated by `hasVisionProvider()` and surfaced as a disabled radio with tooltip when no vision-capable model is configured). New `captureElementCrop()` helper in `pageCapture.js` for baseline element thumbnails. New `backend/tests/self-healing-vision.test.js` with 19 unit tests covering all stages, gating, throws, and budget-circuit-breaker paths. Operator guide rewritten with architecture diagram, cost model, budget mechanics, audit-log event names, baseline capture, and incident-disable runbook (`docs/guide/vision-healing.md`). Cost implications: stage 7 is free CPU; stage 8 calls a paid LLM bounded by per-project daily-call + monthly-USD caps, defaults 100 calls/day + $50/month per project. **Configuration plane is fully wired; the pixelmatch CV implementation + green-run baseline capture lifecycle + SQLite budget counter land in MNT-001b — until then, `tryVisionHeal` returns null in production and tests stay marked broken (existing behaviour) when DOM strategies all fail.**
