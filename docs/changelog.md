...
### Security
- **AUTO-009h hardening** — `file://` server-coverage path safety. The `serverCoverageEndpoint` PATCH validator (`backend/src/routes/projects.js`) now rejects `..` traversal segments in `file://` paths and optionally enforces an operator-configured path-prefix allowlist via the new `COVERAGE_FILE_PATH_PREFIX` env var (comma-separated list of allowed absolute prefixes). Runtime snapshot path (`backend/src/pipeline/serverCoverageProxy.js`) re-validates the same rules so DB tampering / env changes between PATCH and runtime are also caught — defense-in-depth. Unset env keeps the pre-fix behaviour (any absolute path accepted), so existing deployments don't break. Updated `docs/guide/coverage-server-side.md` § File-watch threat surface to document the new env var with a Docker Compose example. Three new test cases in `backend/tests/server-coverage-proxy.test.js` cover `..` rejection, relative-path rejection, and the prefix allowlist round-trip.
