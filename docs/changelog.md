...
### Added
- **Web Vitals Budgets (AUTO-017)**: Per-project Web Vitals budgets (`{ lcp, cls, inp, ttfb }`) with CRUD endpoints under `/api/v1/projects/:id/web-vitals-budgets` (`qa_lead`+ on PATCH/DELETE). At capture time, `captureWebVitals(page)` injects the `web-vitals` library and records LCP/CLS/INP/TTFB per page snapshot (`backend/src/runner/pageCapture.js`, `backend/src/runner/executeTest.js`). On run completion, `evaluateWebVitalsBudgets()` in `backend/src/testRunner.js` produces `{ passed, violations: [{ rule, threshold, actual, testId, testName }] }` and persists it on the run record (migration `015_web_vitals_budgets.sql` adds `projects.webVitalsBudgets` and `runs.webVitalsResult`). The trigger response and callback payload (`backend/src/routes/trigger.js`) include `webVitalsResult` so CI consumers can fail the build on budget violation. `StepResultsView.jsx` renders an inline "Web Vitals Budget Failed" panel on RunDetail when violations exist. Pre-AUTO-017 runs persist `webVitalsResult: null` and render unchanged. (#8)
