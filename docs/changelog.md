...
- **B4.3 — Drop deprecated columns** — Migration 053 drops `agent_configs.fallbackRole` (deprecated in B3.2; route-level `fallbackRouteId` is canonical). Cleans up orphaned `api_keys` rows with `provider LIKE 'compat:%'` (data already migrated by `compat-to-routes.js`).
