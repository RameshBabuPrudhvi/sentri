...
- **Refactor** — Test Lab page (`frontend/src/pages/TestLab.jsx`) trimmed from 1,907 → ~1,140 lines (audit §3.1). Pass 1 extracted presentational helpers — `ProjIcon`, `PipelinePanel` + `PIPELINE_STAGES`, `LiveLog`, `QueueRow` (under `frontend/src/components/test-lab/`); sessionStorage helpers (`loadPersistedRun` / `persistRun` / `clearPersistedRun` / `STORAGE_KEY` / `LOG_CAP`) into `frontend/src/utils/testLabPersistence.js`; attachment MIME guard + size caps into `frontend/src/utils/testLabAttachments.js`. Pass 2 split the three biggest remaining render blocks: `<TestLabTopBar>` (brand + tablist + Record CTA), `<TestLabProjectSidebar>` (left rail), and `<TestLabRunCenter>` (the ~215-line IIFE for the attached-run middle column — banners, inner-tabs, 3-sub-column pipeline view). `QueueTab` now imports `QueueRow`, `EmptyState`, and the `Clock` icon directly instead of receiving them via DI props. Page render now reads as a state machine — `activeRun ? <RunCenter /> : <ConfigPanel />`. The G9 parallel-runs SSE handler + state Maps remain in-file pending that migration's completion. No behaviour change — pure decomposition.
