...
### Changed
- **Dashboard**: Added AUTO-016b accessibility visibility features — CrawlView now shows a per-page accessibility violations panel (severity, WCAG criterion, expandable node selector list) when a site-map node is selected, and the dashboard now includes a "Top Accessibility Offenders" rollup sourced from a new `topAccessibilityOffenders` field on `GET /api/v1/dashboard` (#124).
