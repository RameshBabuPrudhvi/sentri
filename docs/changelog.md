...
### Changed
- **INF-008 (BREAKING for operators):** Default local/deployment database configuration now points to PostgreSQL (`DATABASE_URL=postgres://...`) across `.env.example`, Docker Compose, and getting-started docs. SQLite remains supported as an explicit escape hatch.
- **INF-008:** Migration ordering now sorts by numeric prefix first (with filename tiebreak), and CI now runs backend tests in a dual-database matrix (`sqlite` + `postgres`) with migration-prefix linting.
