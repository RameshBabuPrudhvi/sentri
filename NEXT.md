...
**Title:** Self-healing telemetry dashboard
**Branch:** `feat/cap-004-healing-dashboard`
**Effort:** S | **Priority:** 🔵 Medium
**All dependencies:** none (promoted per rotation rule after PR #12 shipped CAP-003)

> CAP-003 (secret scanner gate on generated tests) ✅ shipped in PR #12 — see Recently completed below. Promoting `CAP-004` from queue slot 2 per `NEXT.md` rotation rule.
...
- [ ] Update this file: move `CAP-004` to "Recently completed", promote the next queue item (slot 2 → Current PR), shift queue items up
- [ ] Add entry to `docs/changelog.md` under `## [Unreleased]`
- [ ] `/healing` page renders per-strategy success rate, top-healed selectors, and savings estimate
- [ ] Sidebar nav entry routes to `/healing` and is gated on the same role as the existing telemetry surfaces
- [ ] `backend/tests/healing-summary.test.js` covers empty-data and populated-histogram cases

---

## ⏭ Queue (next 3 PRs after current)

### 2 · MET-001 — Shared time-series metrics table + `<TrendChart>` component
...
### 3 · PROC-002 — Sprint-tracker hand-off automation
