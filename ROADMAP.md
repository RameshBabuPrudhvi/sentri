...
| ID | Title | PR / Commit                                                     |
|----|-------|-----------------------------------------------------------------|
| AUTO-023 | Autonomous multi-agent collaboration — final bundle. Closes the AUTO-023 roadmap (envelope schema → linear handoff → reviewer↔author loop → supervisor orchestrator → shared memory + tool calling). Bundle 5 ships a closed-set tool registry (`db.listExistingTests`, `db.getTest`, `crawl.getPageHtml`, `playwright.dryRun`, `thread.askPeer`) with per-role visibility (`TOOL_ROLES` intersected with workspace `agent_configs.allowedTools`), Zod-validated structured args + errors (`err.issues[]` with `{path, message, code}` per ZodError), a thread-scoped blackboard (`agent_thread_state` migration 063, optimistic CAS, 64KB cap), envelope-mediated server-side dispatch (`tool_call` / `tool_result` intents, 30s per-call timeout, `agent_tool_calls_total{tool,outcome}` counter), peer Q&A with timeout + per-thread nesting cap + cross-process Redis bridge, workspace scoping at the repo layer (`projectRepo.getByIdInWorkspace` + `run.workspaceId` checks), and a colour-coded UI tool-call timeline (`.ac-tool-chip--{call,success,error}`). Production wire-up: author dispatches `db.listExistingTests` before prompt construction in `journeyGenerator.generateFromDescription` so the LLM sees up to 30 existing test names (catalog dedup at generation time); reviewer dispatches `playwright.dryRun` through the tool registry in `feedbackLoop.runReviewerAuthorLoop` so the static check increments the metric + surfaces on the UI timeline. **Hardening:** per-tool rate limit (`AGENT_TOOL_RATE_LIMIT_PER_MIN` default 60, hard ceiling 1000) sliding window per `(workspaceId, runId, tool)` with Redis-backed + in-memory paths; SQL `LIMIT` pushed to repo via new `testRepo.getRecentByProjectId(projectId, limit)`; tool args scrubbed for secrets via the existing CAP-003 `secretScanner` before envelope persistence; `AbortSignal` threaded from orchestrator + production wire-ups into `executeToolCall` so user-cancelled runs don't burn the 30s tool timeout; bounded exponential-backoff retry (2 attempts, 100ms base + jitter) on transient errors (`SQLITE_BUSY`, `ECONNRESET`, `ETIMEDOUT`) for idempotent reads. Closes the AUTO-023 plan in [`docs/roadmap/autonomous-multi-agent.md`](./docs/roadmap/autonomous-multi-agent.md) — every roadmap checkbox ticked. | PR #38 |
