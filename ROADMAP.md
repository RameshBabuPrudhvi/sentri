...
>
> **Current sprint:** `DIF-015c` Gaps 2/3/5/6 (Recorder gaps completion — point-and-click assertion UX, pause/resume/undo, device profile, stealth launch) — promoted per `NEXT.md` rotation after `AUTO-010` shipped in PR #6. AUTO-010 added a deterministic root-cause clusterer (`backend/src/pipeline/failureClusterer.js`) grouping failed results by normalised error fingerprint, URL origin prefix, and selector edit-distance — no DB, no LLM; `runs.rootCauses` persisted via migration `027_run_root_causes.sql`; called from both the single-process tail in `testRunner.js` AND `finalizeShardedRun` in `runWorker.js` (CAP-002 parity); `RunDetail.jsx` renders a collapsible "Root Cause Summary" panel that auto-expands when ≥2 clusters surface. AUTO-008 (distributed runner across machines, generalising CAP-002's shard pattern to multi-host worker pools) holds queue slot 1, SEC-004 (MFA) slot 2, SEC-006 (PII firewall) slot 3 · **Blockers:** none remaining · **Remaining:** ~17 planned items across Phases 2–5 + Maintenance — see the Summary table at the bottom of this document for the authoritative breakdown. Recent ships: DIF-012 ✅ PR #2 (multi-environment support — new `environments` table per project keyed on `(projectId, name)` with `baseUrl` + AES-encrypted `credentials`, `runs.environmentId` audit column, project-scoped CRUD endpoints with `baseUrl` SSRF validation, `sanitiseEnvCredentialsForClient` strips plaintext passwords from every API response, single shared `utils/envScope.js` `envScopedProject(project, environment, { previewUrl })` helper used by `/run` + `/crawl` + `/tests/generate` + `/record` + `/trigger` + Vercel/Netlify webhook crawl + BullMQ worker so every execution path applies the override identically and `project.url` / `project.credentials` are never mutated, `canonicalUrl` stamped to preserve AUTO-015 production-baseline integrity, env credentials assigned verbatim through the helper because the env repo only `JSON.parse`s on read, run-scope validation rejects cross-project / unknown envId with `400 invalid environmentId` BEFORE the no-approved-tests check; frontend `EnvironmentsTab` on ProjectDetail with admin-only mutation gating, environment dropdowns on RunRegressionModal / RecorderModal / TestLab that reset on project change to prevent stale envId leaks, new Dashboard "Environments" panel showing per-environment pass rate over a 90-day window with batched `environmentRepo.listByProjectIds()` aggregation; `backend/tests/environments.test.js` covers CRUD round-trip, AES round-trip of credentials, PATCH-without-credentials preserves stored secret, explicit `credentials: null` clears it, cross-workspace ACL, cross-project envId rejection on `/run` / `/generate` / `/record`, zero-regression no-envId path, project.url invariant across every mutation; `tests/e2e/specs/environments-ui.spec.mjs` Tier-2 covers the EnvironmentsTab + RunRegressionModal dropdown via `page.route()` mock); CAP-001 ✅ PR #1 (data-driven test fixtures — `test_fixtures(testId, version, format, rows, createdAt)` table keyed on `(testId, version)` with single-quoted `format` CHECK literal so the migration parses on both SQLite and Postgres; per-project `iterationCap` column with `clampIterationCap` `[1, 100]` runtime enforcement; `POST/GET /api/v1/tests/:testId/fixtures` registered in `permissions.json`; `executeTestIterations` substitutes `{{column}}` placeholders per row, `testRunner.js` resolves fixtures keyed to current `codeVersion` (zero-regression for fixture-less tests) and surfaces every iteration to the run aggregator exactly once after retries resolve, fixing a double-count bug where `processResult` inside the retry callback corrupted `run.passed`/`run.failed`; visible `↻ Skipping retry for <test>` log line when a data-driven test fails (data-driven tests don't retry); RFC 4180-aware CSV parser; `TestFixturePanel` on `TestDetail.jsx` with same-version overwrite confirm prompt; `IterationCapPanel` under new **Iterations** tab on `ProjectQualityCard`; `iteration #N` badge in `StepResultsView`; `backend/tests/fixture-iteration.test.js` + `test-fixtures-routes.test.js` + `tests/e2e/specs/test-fixtures-ui.spec.mjs`); AUTO-004 ✅ PR #18 (test impact analysis — pure helper `computeImpactedTests({ tests, changedFiles, changedPages, routeMap })` + `routePrefixesForChangedFiles()` derive URL route prefixes from file paths via heuristic with optional `routeMap` override, `getChangedFilesForPr` paginates the GitHub PR-files API reusing the TTL-cached installation-token path, `trigger.js` filters the dispatched queue to impacted tests and pre-seeds non-impacted approved tests as `{ status: "skipped", skipReason: "skipped_no_impact" }` so every approved test has a resolution, `riskScorer` adds `+10` file-affinity boost composing with AUTO-002's `changedPages[]` boost, migration 022 persists `runs.changedFiles` + `runs.impactAnalysis` (JSON) for the new RunDetail Impact scope panel — deliberate two-column extension over NEXT.md's single-column spec for audit visibility; adjacent: `scoreTestRisk` history filter broadened to exclude all `status === "skipped"` rows so `skipped_no_impact` rows get same treatment as `over_budget`); INT-002b ✅ PR #17 (GitHub integration polish — OAuth-style installation flow with `state` JWT + Redis-backed one-shot nonce, `GET /api/v1/integrations/github/install/start/:projectId` + `install/callback` + `POST /app-webhook` HMAC-verified App-level webhook receiver, `installation.deleted` disables every `github_check_settings` row + emits `integration.github.disabled` activity, `installation_repositories.removed` narrows to `(installationId, repo)`, `installation.{created,suspend,unsuspend}` log-and-no-op, trigger path early-ignores when `enabled=0` so stale-install 401-spam stops; both `TODO(INT-002b):` markers removed); AUTO-001 + INT-002 ✅ PR #15 (risk-based test selection / ordering — pure-function `riskScorer.js` with pass-rate / recency / heal-count / `changedPages` weighting, `normalizeBudgetMinutes()` 240-minute clamp, smoke-test pin, runner + worker dispatch reordering with audit-preserving persisted order, skipped-over-budget pre-seeded results, `RunDetail.jsx` riskScore chip + budget badges, trigger-token path byte-aligned with JWT path; INT-002 GitHub App Check Run client with TTL-cached installation tokens + bounded retry, native `queued → in_progress → success/failure/neutral` lifecycle, HMAC-verified `POST /trigger/github` endpoint with event filtering, `X-GitHub-Delivery` UUID idempotency via cross-dialect `LIKE`-based lookup — Postgres-safe, regressed-tests-only summary with explicit fallback, separate Web Vitals violations bullet, per-project Settings → Integrations tab); AI-001 ✅ PR #14 (generic OpenAI-compatible provider slots `compat:<id>` with SSRF-guarded per-call fetch, TTL cache + Redis pub/sub invalidation, per-slot circuit breakers, Settings UI); AUTO-002 + AUTO-015 + AUTO-002b + AUTO-015b ✅ PR #12 (diff-aware crawling for link-crawl AND state-explorer modes via composite-key baselines, Vercel/Netlify webhook triggers with HMAC verification, "Last deployment run" badge); AUTO-003 + AUTO-003b ✅ PR #10 (confidence-based auto-approval + provenance / revoke / audit trail); AUTO-017.3 + PROC-001 ✅ PR #9 (Web Vitals trend charts + no-orphan-routes CI guard); CAP-004 + MET-001 ✅ PR #8 (self-healing dashboard + time-series metric primitive); CAP-003 ✅ PR #12; UI-REFACTOR-001 ✅ PR #6; DIF-015b Gap 3 + DIF-015c Gap 1 ✅ PR #11; AUTO-019 ✅ PR #10; DIF-005 ✅ PR #9; AUTO-017 ✅ PR #8. PROC-002 + PROC-003 (sprint-promotion automation, originally PR #8 / PR #9) reverted in PR #10 — see Completed Work Summary row.
...
| Phase 4 — Autonomous Intelligence | Risk-based testing, change detection, quality gates | 🔄 In progress — AUTO-001/002/002b/003/003b/004/005/006/007/010/012/013/015/015b/016/016b/017/017.3/019 ✅ (AUTO-010 shipped in PR #6, AUTO-004 in PR #18, AUTO-001 in PR #15); remaining: AUTO-008, AUTO-009, AUTO-011, AUTO-014, AUTO-018, AUTO-021 (AUTO-020 superseded by AUTO-015) · Capabilities row: CAP-001 (data-driven) ✅ PR #1, CAP-002 (sharding) ✅ PR #3; CAP-002b (SaaS-readiness follow-ups) tracked separately in Summary | 14–18 weeks |
...
3. **`installationId` stored in plaintext.** ~~WONTFIX~~ → **✅ Implemented (this PR, override).** Encrypted at rest via AES-256-GCM. See "Reversal of prior WONTFIX" below for the rationale, the cost we knowingly accepted, and the threat-model honesty preserved verbatim from the original analysis.

   **Reversal of prior WONTFIX.** The original analysis (preserved below) concluded encryption was security theater — the threat model is unchanged and still correct. The reversal is purely compliance-driven: rather than wait for SOC 2 / ISO 27001 / HIPAA pursuit to land an "encrypt all third-party identifiers at rest" acceptance criterion (the original compliance tripwire), this PR pre-empts the tripwire so the audit conversation never starts. The decision was taken with eyes open: this is checkbox hardening, not load-bearing security. The threat-model analysis stays in the ROADMAP because future reviewers must understand we ship this against the threat model, not because of it.

   **Implementation.** New `encryptString` / `decryptString` helpers in `backend/src/utils/credentialEncryption.js` use the existing AES-256-GCM machinery with a version-prefixed format (`enc:v1:<iv>:<tag>:<ciphertext>`). `decryptString` returns input unchanged when the prefix is absent — legacy plaintext rows decrypt transparently and re-encrypt naturally on next `upsert()`, so no backfill migration is required. `githubCheckSettingsRepo.upsert` encrypts on write; `rowToSettings` decrypts on every read.

   **Cost we knowingly accepted.** AES-GCM is non-deterministic (random IV per encryption), so the encrypted ciphertext for the same plaintext differs every write. SQL `WHERE installationId = ?` predicates against the ciphertext column can never match. `getByInstallationId`, `disableByInstallationId`, and `disableByRepo` now load every `github_check_settings` row, decrypt the column, and JS-filter — O(n) over the number of projects with GitHub integration configured (one row per project). At current scale this is negligible (≪10ms even at 1000 projects). The escape hatch when row counts grow past ~10k is a deterministic HMAC-based `installationIdHash` lookup column alongside the encrypted blob; not built now to avoid scope creep, but documented in `githubCheckSettingsRepo.js`'s module doc.

   **Threat-model honesty preserved (unchanged from original analysis).** `installationId` is consumed at `backend/src/integrations/githubChecks.js:108-116` only as a path param in `/app/installations/{id}/access_tokens`. It is **not** the auth secret — the GitHub App private key (`GITHUB_APP_PRIVATE_KEY`, env var) is. Knowing the installationId without the private key gives an attacker nothing: the token-exchange call returns 401 without a valid RS256 JWT signed by the App's private key. GitHub itself exposes the installationId publicly: it's in the install-page URL (`github.com/settings/installations/<id>`), in every webhook payload (`installation.id`), and in the App's own admin UI. The encryption added here does not change that — it satisfies a compliance checkbox, not a real attack vector. In every plausible DB-compromise scenario, the attacker also has access to the `CREDENTIAL_SECRET` env var (same deployment, same operator) — encryption with that key adds zero defense-in-depth. Industry comparison: GitLab CI tokens, BuildKite installation IDs, Vercel team IDs, Linear/Jira workspace IDs — none of them encrypt the platform-connection identifier at rest; they encrypt the *credential*, not the *identifier*. We're deliberately ahead of industry practice here because the marginal cost was acceptable and the future-audit cost was higher.

   **No INT-002c follow-up.** The compliance tripwire is resolved by this PR. SOC 2 / ISO 27001 / HIPAA pursuits no longer need to re-open this finding.
...
3. ~~**WONTFIX `installationId` encryption**~~ → **Implement compliance encryption (this PR override).** Add version-prefixed `encryptString` / `decryptString` to `credentialEncryption.js`; encrypt on write in `githubCheckSettingsRepo.upsert`, decrypt in `rowToSettings`. Accept O(n) load-and-filter on the three installation-keyed lookups (`getByInstallationId`, `disableByInstallationId`, `disableByRepo`) — see "Reversal of prior WONTFIX" above for the rationale and the deterministic-HMAC escape hatch if row counts ever grow past ~10k.
