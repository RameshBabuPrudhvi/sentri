...
> ⚠️ **Scope note:** environment `credentials` are stored AES-encrypted but are NOT yet applied to the crawl/run login flow — only `environment.baseUrl` overrides `project.url` at run time (`backend/src/routes/trigger.js`). The project's own `credentials` are still used for authentication. The credentials field exists for forward-compat with DIF-010 (multi-auth profiles); do not assume setting them switches the login. File any production-blocking need as a DIF-010 follow-up.
