...
// ─── Crawl View Component ─────────────────────────────────────────────────────
const PIPELINE_STAGES = [
  {
    label: "Crawl & Snapshot Pages",
    icon: "🔍",
    check: (run) => (run.pagesFound || 0) > 0 || (run.logs || []).some(l => l.includes("smart crawl done") || l.includes("Visiting")),
  },
  {
    label: "Filter Elements",
    icon: "🧹",
    check: (run) => (run.logs || []).some(l => l.includes("ilter") || l.includes("noise")),
  },
  {
    label: "Classify Intents & Journeys",
    icon: "🧠",
    check: (run) => (run.logs || []).some(l => l.includes("lassif") || l.includes("Journey") || l.includes("journey") || l.includes("intent")),
  },
  {
    label: "Generate Tests via AI",
    icon: "⚡",
    check: (run) => (run.logs || []).some(l => l.includes("enerating") || l.includes("Raw tests")),
  },
  {
    label: "Deduplicate Tests",
    icon: "🚫",
    check: (run) => (run.logs || []).some(l => l.includes("edup") || l.includes("duplicate") || l.includes("unique tests")),
  },
  {
    label: "Enhance Assertions",
    icon: "✨",
    check: (run) => (run.logs || []).some(l => l.includes("nhanc") || l.includes("assertion") || l.includes("strengthened")),
  },
  {
    label: "Done",
    icon: "🎉",
...
function CrawlView({ run, isRunning, onBack, fmtMs }) {
  const [logsOpen, setLogsOpen] = React.useState(isRunning); // open while running, collapsed after
  const logRef = React.useRef(null);

  // Auto-open logs when run starts, auto-collapse when done
  React.useEffect(() => {
  ...
  const stats = [
    { label: "Pages Found",         val: run.pagesFound ?? ps.pagesFound,         color: "var(--accent)" },
    { label: "Tests Generated",     val: run.testsGenerated ?? ps.rawTestsGenerated, color: "var(--green)" },
    { label: "Duplicates Removed",  val: ps.duplicatesRemoved,                     color: "var(--amber)" },
    { label: "Journeys Detected",   val: ps.journeysDetected,                      color: "#a855f7" },
    { label: "Assertions Enhanced", val: ps.assertionsEnhanced,                    color: "var(--blue)" },
    { label: "Avg Quality Score",   val: ps.averageQuality != null ? `${ps.averageQuality}/100` : null, color: (ps.averageQuality || 0) >= 60 ? "var(--green)" : "var(--amber)" },
  ];
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: 16, alignItems: "start" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        {/* Pipeline Steps card — always visible */}
          ...
          <div style={{ padding: "10px 18px 0", background: "var(--bg2)" }}>
            <div style={{ height: 4, background: "var(--bg3)", borderRadius: 99, overflow: "hidden", marginBottom: 14 }}>
              <div style={{
                ...
                background: run.status === "completed" ? "var(--green)" : "var(--accent)",
                ...
                <div style={{ position: "relative", flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <span style={{
                      ...
                      color: stage.done ? "var(--text)" : stage.active ? "var(--blue)" : "var(--text3)",
                    ...
                    {stage.active && (
                      <span style={{
                        ...
                        borderRadius: 99, background: "var(--blue-bg)",
                    ...
                    {stage.done && i === stages.length - 1 && !isRunning && (
                      <span style={{ fontSize: "0.65rem", fontWeight: 700, padding: "1px 7px", borderRadius: 99, background: "var(--green-bg)", color: "var(--green)", border: "1px solid #86efac" }}>
                        Complete
                      </span>
                    )}
                  </div>
        ...
        {/* Logs card — collapsible, auto-open while running */}
        <div className="card" style={{ overflow: "hidden" }}>
          {/* Toggle header */}
          <button
          ...
          {[
            {
              ...
              val: (
                <span className={`badge ${isRunning ? "badge-blue" : run.status === "completed" ? "badge-green" : "badge-red"}`}>
                  {isRunning ? <><RefreshCw size={9} style={{ animation: "spin 1s linear infinite" }} /> Running</> : run.status}
                </span>
              ),
            },
            {
              label: "Started",
              val: <span style={{ fontSize: "0.78rem", color: "var(--text2)", fontFamily: "var(--font-mono)" }}>{new Date(run.startedAt).toLocaleTimeString()}</span>,
            },
            {
              label: "Duration",
              val: <span style={{ fontSize: "0.78rem", color: "var(--text2)", fontFamily: "var(--font-mono)" }}>{run.duration ? fmtMs(run.duration) : isRunning ? "…" : "—"}</span>,
            },
          ].map((row, i) => (
...
tracing.start({ screenshots: true, snapshots: true })`}</pre>
      ...
      {isCrawl && (
        <CrawlView run={run} isRunning={isRunning} onBack={() => navigate(-1)} fmtMs={fmtMs} />
      )}
